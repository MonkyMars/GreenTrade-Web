 ...
 */
export const getFavorites = async (userId: string): Promise<FetchedListing[]> => {
  ...
  try {
    ...
    if (!response.data || !response.data.success) {
      throw new AppError(response.data?.message || 'Failed to fetch favorites', {
        code: 'FETCH_FAILED',
    ...
    const errorMessage = appError.message || 'Failed to fetch your favorites. Please try again later.';
    
    // Show error to user in production if not already shown by retryOperation
    if (process.env.NODE_ENV === 'production' && !appError.context?.includes('Fetching favorites')) {
