...
const BrowserComponent: NextPage = () => {
	...
	useEffect(() => {
		...
		const category =
			(searchParams.get('category') as Categories['name']) || 'All Categories';
		...
		if (startPage > 1) {
			buttons.push(
				<button
					key='start'
					onClick={() => goToPage(1)}
					className='relative inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800'
				>
					1
				</button>
			);
			if (startPage > 2) {
				buttons.push(
					<span
						key='start-ellipsis'
						className='relative inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm font-medium text-gray-700 dark:text-gray-200'
					>
						...
					</span>
				);
	...
	return (
		<main className='pt-16 bg-gray-50 dark:bg-gray-900 min-h-screen'>
			{/* Browse Header */}
			<header className='bg-white dark:bg-gray-900 shadow-md sticky top-16 z-20 border-b border-gray-200 dark:border-gray-800'>
			...
			<div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6'>
				<div className='lg:grid lg:grid-cols-12 lg:gap-8'>
					...
					>
						{/* Mobile Filter Panel */}
						<div
							className={`fixed inset-y-0 left-0 w-full max-w-xs bg-white dark:bg-gray-800 shadow-xl z-50 transform transition ease-in-out duration-300 lg:hidden ${isFilterOpen ? 'translate-x-0' : '-translate-x-full'}`}
						>
							<div className='h-full flex flex-col overflow-y-auto p-6'>
								<div className='flex items-center space-x-2 mb-4'>
									<FaFilter className='text-green-600 dark:text-green-500' />
									<h2 className='text-lg font-medium text-gray-900 dark:text-white'>
										Filters
									</h2>
									<button
										onClick={() => setIsFilterOpen(false)}
										className='text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 p-1 rounded-full ml-auto'
									>
								...
								<div className='space-y-6 flex-grow'>
										...
										<div className='space-y-1'>
											{categories.map((category) => (
												<button
													...
													className={`flex w-full items-center py-2 px-3 rounded-md transition-colors ${filters.category === category.name ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 'hover:bg-gray-50 dark:hover:bg-gray-700/30 text-gray-700 dark:text-gray-300'}`}
												>
													<category.icon
														className={`flex-shrink-0 h-5 w-5 mr-3 ${filters.category === category.name ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}
													/>
									...
									<div>
										<label
											htmlFor='minEcoScore-mobile'
											className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'
										>
											Minimum Eco Score
										</label>
										<select
											id='minEcoScore-mobile'
									...
									<div>
										...
										>
											...
											{availableCountries.map((country) => (
												<option key={country + '-mobile'} value={country}>
													{country}
												</option>
											))}
										</select>
						...
						<div className='hidden lg:block lg:sticky lg:top-36 self-start bg-white dark:bg-gray-900 rounded-xl shadow-md p-6 border border-gray-200 dark:border-gray-800'>
							<div className='flex items-center space-x-2 mb-4'>
								<FaFilter className='text-green-600 dark:text-green-500' />
								<h2 className='text-lg font-medium text-gray-900 dark:text-white'>
									Filters
							...
							<div className='space-y-6'>
									...
									<div className='space-y-1'>
										{categories.map((category) => (
											<button
												...
												onClick={() => toggleCategory(category.name)}
												className={`flex cursor-pointer w-full items-center py-2 px-3 rounded-md transition-colors ${filters.category === category.name ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 'hover:bg-gray-50 dark:hover:bg-gray-700/30 text-gray-700 dark:text-gray-300'}`}
											>
												<category.icon
													className={`flex-shrink-0 h-5 w-5 mr-3 ${filters.category === category.name ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}
												/>
								...
								<div>
									<label
										htmlFor='minEcoScore-desktop'
										className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'
									>
										Minimum Eco Score
									</label>
									<select
										id='minEcoScore-desktop'
										value={filters.minEcoScore || '0'}
										onChange={(e) =>
											handleFilterChange(
												'minEcoScore',
								...
								<div>
									...
									>
										<option value='all'>All Countries</option>
										{availableCountries.map((country) => (
											<option key={country + '-desktop'} value={country}>
												{country}
											</option>
										))}
									</select>
					...
					<div className='mt-6 lg:mt-0 lg:col-span-9'>
						{isInitialLoad ? (
							...
							<div className='bg-white dark:bg-gray-900 rounded-xl shadow-md p-6 border border-gray-200 dark:border-gray-800 text-center py-12'>
