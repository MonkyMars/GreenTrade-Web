...
const ActivityTabs: React.FC<ActivityTabsProps> = ({
	...
	const baseTabClasses =
		'flex items-center gap-2 px-4 py-2 rounded-t rounded-b-none border-b-2 transition-all duration-200 text-md';
	const activeTabClasses =
		'text-green-600 dark:bg-green-900/20! bg-green-300/20! border-green-600 dark:text-green-400 dark:border-green-400 font-medium';
	...
	return (
		<div className='bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow'>
			<h2 className='text-xl font-bold text-gray-900 dark:text-white mb-6 flex items-center'>
				<FiPackage className='mr-3 h-5 w-5 text-green-600 dark:text-green-500' />
				Your Activity
			</h2>
			...
			>
				<TabsList className='flex items-center overflow-x-auto border-b border-gray-200 dark:border-gray-700 mb-4 bg-transparent p-0 justify-start w-full sm:w-auto no-scrollbar'>
					<TabsTrigger
						...
						className={cn(
							baseTabClasses,
							activeTab === 'listings' ? activeTabClasses : inactiveTabClasses
						)}
					>
				...
				<TabsContent value='listings' className='pt-2'>
					{userListings && userListings.length > 0 ? (
						...
						<div className='text-center py-10 border border-dashed border-gray-200 dark:border-gray-800 rounded-md bg-gray-50 dark:bg-gray-900/50'>
							...
							<Button
								variant='default'
								className='mt-4 bg-green-600 hover:bg-green-700 text-white border border-green-600 hover:border-green-700 dark:bg-green-700 dark:hover:bg-green-800 dark:border-green-700 dark:hover:border-green-800 transition-colors shadow-sm hover:shadow-md'
							>
						...
						<div className='space-y-4'>
							{userReviews.map((review) => (
								<div key={review.id}>
									<ReviewCard
										review={review}
										className='border border-gray-200 dark:border-gray-800 hover:border-green-200 dark:hover:border-green-800 transition-colors shadow-sm hover:shadow-md'
									/>
								</div>
							))}
						</div>
